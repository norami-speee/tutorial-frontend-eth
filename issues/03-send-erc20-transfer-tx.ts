...
import metaCoinJSON from "../truffle/MetaCoin/build/contracts/MetaCoin.json";
...
describe("send-erc20-transfer-tx", () => {
  ...
  test("send tx", async () => {
    console.log("start");
    const value = 1;
    const beforeSenderCoin = Number(await contract.methods.getBalance(senderAddress).call());
    const beforeReceiverCoin = Number(await contract.methods.getBalance(receiverAddress).call());
    const sendCoin = contract.methods.sendCoin(receiverAddress, value);
    // const encodedAbi = await sendCoin.encodeABI();
    // const gas = await sendCoin.estimateGas();
    const result = await sendCoin.send();
    const afterSenderCoin = Number(await contract.methods.getBalance(senderAddress).call());
    const afterReceiverCoin = Number(await contract.methods.getBalance(receiverAddress).call());
